...
export interface AidDiscoveryFilters extends Pick<
    AidSearchFilters,
    | 'center'
    | 'radiusMeters'
    | 'category'
...
export interface DirectoryDiscoveryFilters extends Pick<
    DirectorySearchFilters,
    'center' | 'radiusMeters' | 'text'
> {
...
export function searchDirectoryResources(
    ...
    return {
        resources: withDirectoryDistance(
            result.items,
            result.distanceMetersByUri,
        ),
    };
}
