...
test('feed discovery defaults to visible items and preserves privacy-safe coordinates', () => {
    const store = new QueryIndexStore(300);
    const visibleUri = `at://did:plc:author/${atLexiconCollections.aidPost}/visible-1`;
    const hiddenUri = `at://did:plc:author/${atLexiconCollections.aidPost}/hidden-1`;
    ...
    store.applyFirehoseEvent(
        normalizeFirehoseEvent({
            ...
            uri: visibleUri,
            record: {
                ...aidRecord('visible-1', 'Visible request'),
                location: {
                    lat: 1.300123,
                    lng: 103.800456,
                    precisionMeters: 120,
    ...
    store.applyFirehoseEvent(
        normalizeFirehoseEvent({
            ...
            uri: hiddenUri,
            record: {
                ...aidRecord('hidden-1', 'Moderated request'),
                location: { lat: 1.3009, lng: 103.8009, precisionMeters: 120 },
            },
        }),
    );
    ...
    store.applyModerationDecision({
        ...
        explanation: 'Safety review',
    });

    const publicFeed = searchFeedCards(store, {
        center: { lat: 1.3, lng: 103.8 },
    });
    assert.deepEqual(
        publicFeed.cards.map(card => card.id),
        ['visible-1'],
    );
    assert.equal(publicFeed.cards[0]?.location?.precisionMeters, 300);
    assert.notEqual(publicFeed.cards[0]?.location?.lat, 1.300123);
    assert.notEqual(publicFeed.cards[0]?.location?.lng, 103.800456);

    const moderationFeed = searchFeedCards(store, {
        includeModerationHidden: true,
        center: { lat: 1.3, lng: 103.8 },
    });

    assert.deepEqual(moderationFeed.cards.map(card => card.id).sort(), [
        'hidden-1',
        'visible-1',
    ]);
    assert.equal(
        moderationFeed.cards.find(card => card.id === 'hidden-1')?.moderation
            ?.visibility,
        'suspended',
    );
});
