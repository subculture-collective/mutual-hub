...
export interface NormalizedFirehoseEvent<C extends AtCollectionName = AtCollectionName> {
  op: FirehoseOperation;
  uri: string;
  repoDid: Did;
  collection: C;
...
export function normalizeFirehoseEvent(event: FirehoseEvent): NormalizedFirehoseEvent {
  ...
  if (!event.record) {
    throw new Error("Firehose create/update events must include a record payload");
  }
