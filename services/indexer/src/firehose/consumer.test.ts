...
test("parseFirehoseUri extracts did, collection, and rkey", () => {
  const parsed = parseFirehoseUri(`at://did:plc:author/${atLexiconCollections.aidPost}/rkey-1`);
...
test("normalizeFirehoseEvent validates create records", () => {
  ...
  assert.equal(normalized.record !== undefined && "id" in normalized.record, true);
