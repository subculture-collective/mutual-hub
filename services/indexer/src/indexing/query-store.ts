import { type AidPostLexiconRecord, atLexiconCollections } from "@mutual-hub/at-lexicons";
import type {
  AidCategory,
  AidPostSummary,
  AidStatus,
...
function addToSetMap<K extends string>(index: Map<K, Set<string>>, key: K, uri: string): void {
  const current = index.get(key) ?? new Set<string>();
  current.add(uri);
  index.set(key, current);
}

function removeFromSetMap<K extends string>(index: Map<K, Set<string>>, key: K, uri: string): void {
...
export class QueryIndexStore {
  ...
  private readonly directoryByUri = new Map<string, IndexedDirectoryResource>();
  ...
  private readonly directoryByType = new Map<DirectoryResourceType, Set<string>>();
  ...
  private upsertAidPost(
    ...
    const normalizedLocation =
      record.location === undefined
        ? undefined
        : normalizeIndexedLocation(record.location, this.minimumPrecisionMeters);
  ...
  private upsertDirectoryResource(
    ...
    const indexedResource: IndexedDirectoryResource = {
      ...
      name: record.name,
      type: record.type,
      location: normalizeIndexedLocation(record.location, this.minimumPrecisionMeters),
  ...
  searchAidPosts(filters: AidSearchFilters = {}): AidSearchResult {
    ...
    if (filters.text && filters.text.trim().length > 0) {
      candidateUris = intersectSets(candidateUris, this.aidTextIndex.search(filters.text));
    }
  ...
  searchDirectoryResources(filters: DirectorySearchFilters = {}): DirectorySearchResult {
    ...
    if (filters.text && filters.text.trim().length > 0) {
      candidateUris = intersectSets(candidateUris, this.directoryTextIndex.search(filters.text));
    }

    const distanceMetersByUri: Record<string, number> = {};
    const filtered = [...candidateUris]
      .map((uri) => this.directoryByUri.get(uri))
      .filter((resource): resource is IndexedDirectoryResource => resource !== undefined)
      .filter((resource) => {
        const distanceMeters = filters.center
          ? haversineDistanceMeters(filters.center, resource.location)
          : undefined;
