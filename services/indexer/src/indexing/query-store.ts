...
export class QueryIndexStore {
    ...
    private readonly moderationByUri = new Map<
        string,
        AidPostModerationState
    >();
    ...
    private upsertAidPost(
        ...
        const moderationState = this.moderationByUri.get(uri) ?? {
            visibility: 'visible',
            appealStatus: 'none',
            updatedAt: indexedAt,
        };
    ...
    private removeAidPost(uri: string): void {
        ...
        this.moderationByUri.delete(uri);
    ...
    searchAidPosts(filters: AidSearchFilters = {}): AidSearchResult {
        ...
        const filtered = [...candidateUris]
            ...
            .filter(post => {
                ...
                if (
                    !filters.includeModerationHidden &&
                    post.moderation &&
                    post.moderation.visibility !== 'visible'
                ) {
                    return false;
                }
    ...
    setAidModerationState(
        uri: string,
        state: Pick<AidPostModerationState, 'visibility'> &
            Partial<Pick<AidPostModerationState, 'appealStatus' | 'updatedAt'>>,
    ): void {
        const current = this.moderationByUri.get(uri);
        const next: AidPostModerationState = {
            visibility: state.visibility,
            appealStatus: state.appealStatus ?? current?.appealStatus ?? 'none',
            updatedAt: state.updatedAt ?? new Date().toISOString(),
        };

        this.moderationByUri.set(uri, next);

        const indexed = this.aidByUri.get(uri);
        if (!indexed) {
            return;
        }

        this.aidByUri.set(uri, {
            ...indexed,
            moderation: next,
        });
    }
    ...
    applyModerationDecision(
        ...
        const current = this.moderationByUri.get(decision.targetUri);
        const visibility =
            decision.action === 'allow' ? 'visible'
            : decision.action === 'delist' ? 'delisted'
            : decision.action === 'suspend_visibility' ? 'suspended'
            : (current?.visibility ?? 'visible');

        this.setAidModerationState(decision.targetUri, {
            visibility,
            updatedAt,
        });
    }
