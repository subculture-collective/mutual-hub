...
export interface IndexerService {
    ...
    ) => ReturnType<typeof searchMapCards>;
    searchFeedCards: (
        filters?: AidDiscoveryFilters,
    ) => ReturnType<typeof searchFeedCards>;
    searchDirectoryResources: (
...
export function createIndexerService(
    ...
    return {
        ...
        ingestFirehoseEvent,
        ingestFirehoseEvents,
        applyModerationDecision: (decision, updatedAt) =>
            store.applyModerationDecision(decision, updatedAt),
        searchMapCards: (filters = {}) => searchMapCards(store, filters),
        searchFeedCards: (filters = {}) => searchFeedCards(store, filters),
        searchDirectoryResources: (filters = {}) =>
            searchDirectoryResources(store, filters),
