...
export type MapTriageAction = "contact_helper" | "view_feed_context" | "mark_in_progress";
...
export function toApproximateMapMarker(card: MapAidCard): MapMarker | undefined {
  if (!card.location) {
    return undefined;
  }
...
export function filterMapCards(
  ...
  return cards.filter((card) => {
    ...
    if (query.since) {
      ...
      if (!Number.isNaN(sinceMs) && (Number.isNaN(updatedMs) || updatedMs < sinceMs)) {
        return false;
      }
    }
...
export function clusterMapCards(cards: readonly MapAidCard[], gridSizeMeters = 1200): MapCluster[] {
  ...
  for (const card of cards) {
    ...
    const lngStep =
      gridSizeMeters /
      (metersPerLatDegree * Math.max(0.2, Math.abs(Math.cos(toRadians(marker.lat)))));
