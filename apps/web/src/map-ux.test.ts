...
test("map clustering groups nearby cards into a single cluster", () => {
  const cards: MapAidCard[] = [
    buildCard({ id: "near-1", location: { lat: 1.3001, lng: 103.8001, precisionMeters: 300 } }),
    buildCard({ id: "near-2", location: { lat: 1.3002, lng: 103.8002, precisionMeters: 300 } }),
  ];
...
test("detail drawer returns triage actions with accessible labels", () => {
  const cards: MapAidCard[] = [buildCard({ id: "drawer-1", title: "Need water" })];
