...
function selectVolunteer(
    ...
    return candidates
        ...
        .sort((left, right) => {
            const distanceOrder = byAscNumber(
                left.distanceMeters,
            ...
            const activityOrder = byDescTimestamp(
                left.lastActiveAt,
                right.lastActiveAt,
            );
            if (activityOrder !== 0) {
...
export function decideChatRoute(input: RoutingDecisionInput): RoutingDecision {
    ...
    const volunteer = selectVolunteer(
        input.volunteerCandidates ?? [],
        input.postCategory,
    );
