...
const atprotoSchema = z.object({
  ATPROTO_SERVICE_DID: z
    .string()
    .regex(/^did:[a-z0-9]+:[a-z0-9._:%-]+$/i, 'ATPROTO_SERVICE_DID must be a valid DID string.'),
...
const moderationWorkerSchema = baseSchema.merge(atprotoSchema).extend({
  ...
  MODERATION_WORKER_CONCURRENCY: z.coerce.number().int().min(1).max(64).default(2)
});
...
const parseEnv = <T extends AnySchema>(scope: string, schema: T): z.infer<T> => {
  loadWorkspaceEnvFiles();
  const result = schema.safeParse(process.env);

  if (!result.success) {
    throw new Error(`Invalid ${scope} configuration: ${formatZodErrors(result.error)}`);
  }
...
export const loadIndexerConfig = (): IndexerConfig => parseEnv('indexer', indexerSchema);
