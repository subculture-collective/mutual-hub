...
describe('P2.2 DID auth/session lifecycle', () => {
  ...
  it('surfaces handle resolution failures with structured error codes', async () => {
    ...
    await expect(
      service.loginWithHandle({ handle: 'alice.mutualhub.test', password: 'pw' })
    ).rejects.toMatchObject({ code: 'HANDLE_RESOLUTION_FAILED' satisfies DidAuthError['code'] });
  });
  ...
  it('refreshes sessions before expiry and returns updated tokens', async () => {
    ...
    const nearExpiry = new Date(new Date(session.expiresAt).getTime() - 5_000);
  ...
  it('fails gracefully when refresh token has expired', async () => {
    ...
    const afterRefreshExpiry = new Date(new Date(session.refreshExpiresAt).getTime() + 1_000);

    await expect(service.refreshIfNeeded(session, afterRefreshExpiry)).rejects.toMatchObject({
