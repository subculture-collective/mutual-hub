...
export interface IdentityProvider {
  ...
  refreshSession(refreshJwt: string): Promise<Pick<DidSession, 'accessJwt' | 'refreshJwt' | 'expiresAt' | 'refreshExpiresAt'>>;
}
...
export class DidAuthService {
  ...
  async loginWithHandle(input: LoginInput): Promise<DidSession> {
    ...
    if (!parsedHandle.success) {
      throw new DidAuthError('INVALID_HANDLE', 'Handle must be a valid DNS-like handle.', {
        handle: input.handle
      });
    }
    ...
    try {
      ...
      throw new DidAuthError('HANDLE_RESOLUTION_FAILED', 'Failed to resolve handle to DID.', {
    ...
    if (!parsedDid.success) {
      throw new DidAuthError('INVALID_DID', 'Resolved DID is malformed.', {
        did: resolution.did,
        handle: parsedHandle.data
      });
    }
    ...
    try {
      ...
      throw new DidAuthError('SESSION_CREATE_FAILED', 'Unable to create DID session.', {
        did: parsedDid.data,
        handle: parsedHandle.data,
        cause: error instanceof Error ? error.message : 'unknown'
      });
  ...
  async refreshIfNeeded(session: DidSession, now = new Date()): Promise<RefreshResult> {
    ...
    if (refreshExpiresAt <= nowMs) {
      throw new DidAuthError('SESSION_EXPIRED', 'Refresh token expired; user must log in again.', {
        did: parsed.did,
    ...
    try {
      const refreshed = await this.provider.refreshSession(parsed.refreshJwt);
      ...
      throw new DidAuthError('SESSION_REFRESH_FAILED', 'Session refresh failed.', {
        did: parsed.did,
        cause: error instanceof Error ? error.message : 'unknown'
      });
    }
