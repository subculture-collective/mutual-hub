...
const didSchema = z.string().regex(/^did:[a-z0-9]+:[a-z0-9._:%-]+$/i, 'Expected a valid DID');
...
const buildAtUri = (authorDid: string, collection: RecordNsid, rkey: string): string => {
...
export class AtRecordRepository {
  ...
  createRecord<N extends RecordNsid>(input: CreateRecordInput<N>): StoredRecord<N> {
    ...
    if (existing?.lifecycle === 'active') {
      throw new RecordWriteError('CONFLICT', 'Record URI already exists.', { uri });
    }

    if (existing?.lifecycle === 'tombstoned') {
      throw new RecordWriteError('TOMBSTONED', 'Cannot recreate a tombstoned record URI.', { uri });
    }
    ...
    try {
      ...
      throw new RecordWriteError('VALIDATION_FAILED', 'Record payload failed schema validation.', {
  ...
  updateRecord(input: UpdateRecordInput): StoredRecord<RecordNsid> {
    ...
    if (!existing) {
      throw new RecordWriteError('NOT_FOUND', 'Record not found.', { uri: input.uri });
    }

    if (existing.lifecycle === 'tombstoned') {
      throw new RecordWriteError('TOMBSTONED', 'Cannot update a tombstoned record.', {
        uri: input.uri,
    ...
    try {
      validated = validateRecordPayload(existing.collection, input.value) as RecordByNsid[RecordNsid];
    } catch (error) {
      throw new RecordWriteError('VALIDATION_FAILED', 'Record payload failed schema validation.', {
    ...
    if (existing.collection === recordNsid.aidPost) {
      ...
      if (!aidPostTransitionMap[previousStatus]?.has(nextStatus)) {
        throw new RecordWriteError('INVALID_TRANSITION', 'Aid post status transition is not allowed.', {
          previousStatus,
          nextStatus,
          uri: input.uri,
          updatedByDid
  ...
  listMutationEvents(): RecordMutationEvent[] {
    return this.mutationEvents.map((event) => JSON.parse(JSON.stringify(event)) as RecordMutationEvent);
  }
  ...
  deserializeMutationEvent(payload: string): RecordMutationEvent {
    return mutationEventSchema.parse(JSON.parse(payload)) as RecordMutationEvent;
  }

  private cloneRecord<N extends RecordNsid>(record: StoredRecord<N>): StoredRecord<N> {
