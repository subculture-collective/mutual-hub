...
const readLexicon = (fileName: string): AtLexiconDoc => {
  const json = JSON.parse(readFileSync(resolve(lexiconDir, fileName), 'utf8'));
...
export const lexiconDocs: Record<RecordNsid, AtLexiconDoc> = {
  ...
  [recordNsid.volunteerProfile]: readLexicon('app.mutualhub.volunteer.profile.v1.json'),
  [recordNsid.conversationMeta]: readLexicon('app.mutualhub.conversation.meta.v1.json'),
  [recordNsid.moderationReport]: readLexicon('app.mutualhub.moderation.report.v1.json'),
  [recordNsid.directoryResource]: readLexicon('app.mutualhub.directory.resource.v1.json')
};
