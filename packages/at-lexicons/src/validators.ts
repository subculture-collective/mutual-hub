...
const atUriSchema = z.string().regex(/^at:\/\/[^\s]+$/i, 'Expected a valid at:// URI');
...
export const volunteerProfileSchema = z.object({
  ...
  capabilities: z.array(z.enum(['transport', 'food-delivery', 'translation', 'first-aid', 'childcare', 'other'])).min(1),
  availability: z.enum(['immediate', 'within-24h', 'scheduled', 'unavailable']),
...
export const directoryResourceSchema = z.object({
  ...
  category: z.enum(['food-bank', 'shelter', 'clinic', 'legal-aid', 'hotline', 'other']),
  ...
  verificationStatus: z.enum(['unverified', 'community-verified', 'partner-verified']),
  createdAt: isoDateTimeSchema,
  updatedAt: isoDateTimeSchema.optional()
});
...
export const recordValidators: { [K in RecordNsid]: z.ZodType<RecordByNsid[K]> } = {
...
export const safeValidateRecordPayload = <N extends RecordNsid>(nsid: N, payload: unknown) => {
