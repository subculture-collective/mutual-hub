...
import { atLexiconCollections, isKnownCollection, validateRecord } from "./index.js";
...
test("known collection helper recognizes supported collection IDs", () => {
  assert.equal(isKnownCollection(atLexiconCollections.conversationMetadata), true);
...
test("validates extended conversation metadata record shape", () => {
  const record = validateRecord(atLexiconCollections.conversationMetadata, {
    ...
    routingRationale: "Verified volunteer selected by deterministic ordering",
