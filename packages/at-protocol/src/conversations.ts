  ...
  return {
    ...
    routing:
      record.routingDestinationType && record.routingRationale
        ...
        : undefined,
    transport: record.transportMode
      ? {
...
export function resolveRecipientTransportCapability(
  ...
  return {
    ...
    fallbackReason,
    notice:
      "Recipient cannot receive AT-native messages right now. Continue only with a safe fallback handoff path.",
  };
}
...
export class ConversationMetadataRepository {
  ...
  upsertMetadata(
    ...
    const uri = createRecordUri(input.repoDid, atLexiconCollections.conversationMetadata, rkey);
    ...
    if (existing) {
      ...
      }) as StoredAtRecord<typeof atLexiconCollections.conversationMetadata>;
    }
  ...
  listForAudit(query: ConversationMetadataAuditQuery = {}): ConversationMetadataAuditRecord[] {
    return this.records
      ...
      .filter(({ record }) => {
        ...
        if (query.transportMode && record.transportMode !== query.transportMode) {
          return false;
        }

        if (query.fallbackOnly && record.transportMode !== "fallback_notice") {
          return false;
        }
