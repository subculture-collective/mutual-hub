...
function normalizeSession(session: AtSession): AtSession {
  ...
  if (
    ...
    throw new Error("AT session timestamps must be valid ISO datetime strings");
  }
...
export class AtAuthService {
  ...
  async refreshSession(currentSession: AtSession): Promise<AtSession> {
    const refreshed = await this.client.refreshSession(currentSession.refreshJwt);
    return normalizeSession(refreshed);
  }

  async ensureFreshSession(currentSession: AtSession, now = Date.now()): Promise<AtSession> {
    if (!isSessionExpiringSoon(currentSession, now, this.refreshSkewSeconds)) {
