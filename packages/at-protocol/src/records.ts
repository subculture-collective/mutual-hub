...
export class AtRecordRepository {
  ...
  createRecord<C extends AtCollectionName>(input: CreateRecordInput<C>): StoredAtRecord<C> {
    const uri = createRecordUri(input.repoDid, input.collection, input.rkey);
  ...
  updateRecord(input: UpdateRecordInput): StoredAtRecord {
    if (this.tombstones.has(input.uri)) {
      throw new Error(`Cannot update tombstoned record at URI: ${input.uri}`);
    }

    const existing = this.records.get(input.uri);
    if (!existing) {
      throw new Error(`Cannot update missing record at URI: ${input.uri}`);
    }
...
export function isRecordValidationError(error: unknown): error is AtRecordValidationError {
